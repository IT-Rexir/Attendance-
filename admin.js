...
(() => {
  ...
  sendWebhookBtn.addEventListener("click", async () => {
    ...
    // Changing 'discord.com' to 'discordapp.com' skips the browser's CORS restriction completely!
    const WEBHOOK_URL = "https://discordapp.com/api/webhooks/1518966001885118625/rCb5480x4HeE23vE4fhXGF6I_osHZHfPdPHMbEM4TKd0eSsKDzAviSZYBtu088s0uN9G";

    try {
      sendWebhookBtn.disabled = true;
      showToast("Sending data...", "ok");

      // Generate text strings for individual students
      const recordLines = records.map(r => 
        `• **${r.name}** (\`${r.lrn}\`) — ${r.grade || ''} ${r.strand || ''} [${r.status}] at *${r.time}*`
      );

      // Split information batches up under 2000 characters
      const batches = [];
